...
if ($_.LastLogonDate  -lt ((Get-Date).AddDays(-90)) -and $_.LastLogonDate -gt ((Get-Date).AddDays(-180)) -and $_.LastLogonDate  -gt 1 -and $_.Enabled -eq $True){
    if (($_.OperatingSystem -like "*Windows 1*") -or ($_.OperatingSystem -like "*Windows 8*")){
         ...
         #Write-Host $_.Name
         $compid = Get-ADComputer -Identity $_.Name
	     Disable-ADAccount -Identity $compid
     }
    }
}
